 ...
 * <p>
 * The only function of interest is {@link #getApi()}, which exposes some
...
public final class RedisBungee extends Plugin {
  ...
  private final Cache<Object, Multimap<String, UUID>> serverToPlayersCache =
   ...
   */
  public static RedisBungeeAPI getApi() { return api; }

  static PubSubListener getPubSubListener() { return psl; }

  final List<String> getServerIds() { return serverIds; }
  ...
  public Set<UUID> getPlayersOnProxy(String server) {
    checkArgument(getServerIds().contains(server),
                  server + " is not a valid proxy ID");
    try (Jedis jedis = pool.getResource()) {
  ...
  final Multimap<String, UUID> serversToPlayers() {
    try {
      return serverToPlayersCache.get(
          SERVER_TO_PLAYERS_KEY, new Callable<Multimap<String, UUID>>() {
            @Override
            public Multimap<String, UUID> call() throws Exception {
              Collection<String> data =
                  (Collection<String>)serverToPlayersScript.eval(
                      ImmutableList.<String>of(), getServerIds());

              ImmutableMultimap.Builder<String, UUID> builder =
  ...
  final int getCount() { return globalPlayerCount.get(); }

  final int getCurrentCount() {
    Long count = (Long)getPlayerCountScript.eval(ImmutableList.<String>of(),
  ...
  final void sendProxyCommand(@NonNull String proxyId,
                              @NonNull String command) {
    checkArgument(getServerIds().contains(proxyId) ||
                      proxyId.equals("allservers"),
                  "proxyId is invalid");
  ...
  public void onEnable() {
    ThreadFactory factory =
        ((ThreadPoolExecutor)getExecutorService()).getThreadFactory();
    ScheduledExecutorService service =
        Executors.newScheduledThreadPool(24, factory);
    ;
    try {
      ...
      getLogger().log(Level.WARNING,
                      "Can't replace BungeeCord thread pool with our own", e);
    }
    try {
      loadConfig();
    } catch (IOException e) {
    ...
    if (pool != null) {
      try (Jedis tmpRsc = pool.getResource()) {
        ...
        tmpRsc.hset("heartbeats", configuration.getServerId(),
                    tmpRsc.time().get(0));
      ...
      heartbeatTask = service.scheduleAtFixedRate(new Runnable() {
        ...
        public void run() {
          try (Jedis rsc = pool.getResource()) {
            ...
            rsc.hset("heartbeats", configuration.getServerId(),
          ...
          try {
            ...
            getLogger().log(
                Level.SEVERE,
                "Unable to update data - did your Redis server go away?", e);
          }
        }
      ...
      if (configuration.isRegisterBungeeCommands()) {
        getProxy().getPluginManager().registerCommand(
            this, new RedisBungeeCommands.GlistCommand(this));
        getProxy().getPluginManager().registerCommand(
            this, new RedisBungeeCommands.FindCommand(this));
        getProxy().getPluginManager().registerCommand(
            this, new RedisBungeeCommands.LastSeenCommand(this));
        getProxy().getPluginManager().registerCommand(
            this, new RedisBungeeCommands.IpCommand(this));
      }
      getProxy().getPluginManager().registerCommand(
          this, new RedisBungeeCommands.SendToAll(this));
      getProxy().getPluginManager().registerCommand(
          this, new RedisBungeeCommands.ServerId(this));
      getProxy().getPluginManager().registerCommand(
          this, new RedisBungeeCommands.ServerIds());
      getProxy().getPluginManager().registerCommand(
          this, new RedisBungeeCommands.PlayerProxyCommand(this));
      getProxy().getPluginManager().registerCommand(
          this, new RedisBungeeCommands.PlistCommand(this));
      getProxy().getPluginManager().registerCommand(
          this, new RedisBungeeCommands.DebugCommand(this));
      api = new RedisBungeeAPI(this);
      getProxy().getPluginManager().registerListener(
          this,
          new RedisBungeeListener(this, configuration.getExemptAddresses()));
      ...
      integrityCheck = service.scheduleAtFixedRate(new Runnable() {
        ...
        public void run() {
          try (Jedis tmpRsc = pool.getResource()) {
            ...
            Set<String> playersInRedis = tmpRsc.smembers(
            ...
            for (String s : lagged) {
              ...
              if (!laggedPlayers.isEmpty()) {
                getLogger().info("Cleaning up lagged proxy " + s + " (" +
                                 laggedPlayers.size() + " players)...");
            ...
            for (String member : absentLocally) {
              ...
              for (String proxyId : getServerIds()) {
                if (proxyId.equals(configuration.getServerId()))
                  continue;
                if (tmpRsc.sismember("proxy:" + proxyId + ":usersOnline",
                                     member)) {
              ...
              if (!found) {
                ...
                tmpRsc.srem("proxy:" + configuration.getServerId() +
                                ":usersOnline",
                            member);
                getLogger().warning(
            ...
            for (String player : absentInRedis) {
              ...
              getLogger().warning("Player " + player +
                                  " is on the proxy but not in Redis.");
            ...
            getLogger().log(Level.SEVERE, "Unable to fix up stored player data",
                            e);
          }
        }
  ...
  public void onDisable() {
    if (pool != null) {
      ...
      try (Jedis tmpRsc = pool.getResource()) {
        ...
        if (tmpRsc.scard("proxy:" + configuration.getServerId() +
                         ":usersOnline") > 0) {
          Set<String> players = tmpRsc.smembers(
              "proxy:" + configuration.getServerId() + ":usersOnline");
  ...
  private void loadConfig() throws IOException, JedisConnectionException {
    ...
    final String redisServer =
        configuration.getString("redis-server", "localhost");
    ...
    if (serverId == null || serverId.isEmpty()) {
      throw new RuntimeException(
          "server-id is not specified in the configuration or is empty");
    }
    ...
    if (redisServer != null && !redisServer.isEmpty()) {
      ...
      FutureTask<JedisPool> task = new FutureTask<>(new Callable<JedisPool>() {
        ...
        public JedisPool call() throws Exception {
          ...
          return new JedisPool(config, redisServer, redisPort, 0,
                               finalRedisPassword);
        }
      });
      ...
      try (Jedis rsc = pool.getResource()) {
        ...
        if (crashFile.exists()) {
          ...
          try {
            ...
            if (redisTime < value + 20) {
              ...
              getLogger().severe(
                  "For data consistency reasons, RedisBungee will now disable itself.");
              getLogger().severe(
        ...
        FutureTask<Void> task2 = new FutureTask<>(new Callable<Void>() {
          ...
          public Void call() throws Exception {
            ...
            RedisBungee.configuration = new RedisBungeeConfiguration(
                RedisBungee.this.getPool(), configuration);
            return null;
          }
        });
    ...
    public void run() {
      ...
      try (Jedis rsc = pool.getResource()) {
        try {
          ...
          getLogger().log(Level.INFO, "PubSub error, attempting to recover.",
                          e);
          try {
            jpsh.unsubscribe();
          } catch (Exception e1) {
        ...
        getLogger().log(Level.INFO,
                        "PubSub error, attempting to recover in 5 secs.");
        getProxy().getScheduler().schedule(
            RedisBungee.this, PubSubListener.this, 5, TimeUnit.SECONDS);
      }
